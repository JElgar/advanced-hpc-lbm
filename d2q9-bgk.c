...
int initialise(const char* paramfile, const char* obstaclefile,
               t_param* params, t_speed** cells_ptr, t_speed** tmp_cells_ptr, t_speed** tmp_cells2_ptr,
...
int timestep(const t_param params, t_speed* cells, t_speed* tmp_cells, t_speed* tmp_cells2, char* obstacles);
int accelerate_flow(const t_param params, t_speed* cells, char* obstacles);
int propagate_rebound_and_collisions(const t_param params, t_speed* cells, t_speed* tmp_cells, t_speed* tmp_cells2, char* obstacles);
...
int main(int argc, char* argv[])
{
  ...
  t_speed* tmp_cells2 = NULL;    /* second scratch space */
  ...
  initialise(paramfile, obstaclefile, &params, &cells, &tmp_cells, &tmp_cells2, &obstacles, &av_vels);
  ...
  {
    timestep(params, cells, tmp_cells, tmp_cells2, obstacles);
...
int timestep(const t_param params, t_speed* cells, t_speed* tmp_cells, t_speed* tmp_cells2, char* obstacles)
{
  ...
  propagate_rebound_and_collisions(params, cells, tmp_cells, tmp_cells2, obstacles);
    ...
    for (int ii = 0; ii < params.nx; ii++)
    {
      ...
      if (obstacles[jj*params.nx + ii])
      {
        ...
        ** mirroring, and writing into main grid */
        cells[ii + jj*params.nx].speeds[0] = cells[ii + jj*params.nx].speeds[0];
        ...
        tmp_cells[ii + jj*params.nx].speeds[1] = cells[ii + jj*params.nx].speeds[1];
        tmp_cells[ii + jj*params.nx].speeds[2] = cells[ii + jj*params.nx].speeds[2];
        tmp_cells[ii + jj*params.nx].speeds[3] = cells[ii + jj*params.nx].speeds[3];
        tmp_cells[ii + jj*params.nx].speeds[4] = cells[ii + jj*params.nx].speeds[4];
        tmp_cells[ii + jj*params.nx].speeds[5] = cells[ii + jj*params.nx].speeds[5];
        tmp_cells[ii + jj*params.nx].speeds[6] = cells[ii + jj*params.nx].speeds[6];
        tmp_cells[ii + jj*params.nx].speeds[7] = cells[ii + jj*params.nx].speeds[7];
        tmp_cells[ii + jj*params.nx].speeds[8] = cells[ii + jj*params.nx].speeds[8];
      } 
...
// Potentially can be combined with other single pass but this only occurs if there is an obstacle
int collision(const t_param params, t_speed* cells, t_speed* tmp_cells, char* obstacles)
{
  const float c_sq = 1.f / 3.f; /* square of speed of sound */
  const float c_2sq = 2.f * c_sq; /* 2 times square of speed of sound */
               ...
               char** obstacles_ptr, float** av_vels_ptr)
{
  ...
  *tmp_cells2_ptr = (t_speed*)malloc(sizeof(t_speed) * (params->ny * params->nx));

  if (*tmp_cells_ptr == NULL) die("cannot allocate memory for tmp_cells", __LINE__, __FILE__);
  if (*tmp_cells2_ptr == NULL) die("cannot allocate memory for tmp_cells", __LINE__, __FILE__);
